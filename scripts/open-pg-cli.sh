...
if [[ ! $PWD =~ "maas-backend/scripts" ]]
  then
    echo "This script can be run only in /maas-backend/scripts" && exit 0
fi
...
while getopts "hs:" opt; do
  case $opt in
    h  ) echo "Usage: bash connect-db.sh -s <STAGE>"; exit 1;;
    s  ) export STAGE=$OPTARG;;
    \? ) echo "Usage: bash connect-db.sh -s <STAGE>"; exit 1;;    :  ) echo "Missing option argument for -$OPTARG" >&2; exit 1;;    *  ) echo "Invalid option: -$OPTARG" >&2; exit 1;;
  esac
done
...
if [[ -z $STAGE ]] && [[ "$STAGE" != "dev" ]] && [[ "$STAGE" != "test" ]] && [[ "$STAGE" != "prod" ]] && [[ "$STAGE" != "alpha" ]];
  then
     ...
     echo "Usage: bash dump-table.sh -s <STAGE>"; exit 1;
fi
...
echo PGPASSWORD=$MAAS_PGPASSWORD pgcli -h $MAAS_PGHOST -d $MAAS_PGDATABASE -p $MAAS_PGPORT -U $MAAS_PGUSER -w
