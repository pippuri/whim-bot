...
newKey() {
  ...
  ssh-keygen -t rsa -b 768 -f latest.key -N ''
...
refresh() {
  ...
  node -p -e "const file=require('../../../_meta/variables/s-variables-$STAGE.json'); file.MAAS_TICKET_PREVIOUS_CYCLE_TIME=$NEXT_REFRESH_DATE;const result=require('fs').writeFileSync('../../../_meta/variables/s-variables-$STAGE.json',JSON.stringify(file, null, 2));'Keys succesfully refreshed'"
}
