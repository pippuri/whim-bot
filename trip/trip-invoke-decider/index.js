...
class Decider {
  ...
  _decide() {
    ...
    let taskName = this.flow.task && this.flow.task.name;
  ...
  _fetchItinerary() {
    ...
    return bus.call(LAMBDA_ITINERARY_RETRIEVE, data)
      .then(response => {
        ...
        let itinerary = response.itineraries && response.itineraries.find(itinerary => {
          return itinerary.id === this.flow.trip.referenceId
        })
        if (itinerary) {
          return Promise.resolve(itinerary);
        } else {
          return Promise.reject(`cannot find itinerary`);
        }
      })
      .catch(err => {
    ...
    itinerary.legs.forEach((leg) => {
      if (leg.bookingId) {
        ...
        if (leg.startTime - (35 * 1000) < this.now) {
          ...
          let timeout = leg.startTime - (30 * 1000);
