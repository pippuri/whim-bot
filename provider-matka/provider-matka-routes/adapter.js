...
function convertKKJ3ToWGS84(x, y) {
  var to = proj4('EPSG:2393', 'WGS84', [x, y]);
  return {
    lon: to[0],
    lat: to[1],
  };
...
function convertFromTo(xy, name) {
  var coords = xy ? convertKKJ3ToWGS84(xy.x, xy.y) : {};
  return {
...
function convertLeg(leg) {
  console.log('Leg:', leg);
  var startTime = leg.DEPARTURE ? convertDateTime(leg.DEPARTURE[0].$.date, leg.DEPARTURE[0].$.time) : undefined;
  var endTime = leg.ARRIVAL ? convertDateTime(leg.ARRIVAL[0].$.date, leg.ARRIVAL[0].$.time) : undefined;
  return {
    startTime: startTime,
    endTime: endTime,
...
function convertItinerary(route) {
  var duration = parseFloat(route.LENGTH[0].$.time) * 60 * 1000;
  var startTime = convertDateTime(route.POINT[0].DEPARTURE[0].$.date, route.POINT[0].DEPARTURE[0].$.time);
