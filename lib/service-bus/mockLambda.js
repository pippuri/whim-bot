...
if ( process.env.maas_test_run && ! process.env.SKIP_LOCAL_MAAS_REPOSITORY_PATHS ) {
  loadLocalMaasRepositoryLambdas( 'LOCAL_MAAS_TRANSPORT_BOOKING_PATH', '../maas-transport-booking', 'maas-transport', {
    ...
    'booking-sixt': ['create', 'options-retrieve', 'retrieve', 'update'],
    'booking-taxi': ['create', 'retrieve', 'update'],
  } );
  loadLocalMaasRepositoryLambdas( 'LOCAL_MAAS_TSP_REFERENCE_PATH', '../maas-tsp-reference', 'maas-tsp-reference', {
...
function invokePromise(functionName, event) {
  return new Promise((resolve, reject) => {
    ...
    if ( localRepositoryLambdaMap.hasOwnProperty(functionName) ) {
      ...
      fsp.writeFile(eventFile, JSON.stringify(event))
        .then( () => {
          return new Promise( (_resolve, _reject) => {
            child_process.exec( `sls function integration-run -s dev ${localRepositoryLambdaMap[functionName].functionName} ${eventFile} ${resultFile}`,
              {
        ...
        .then( () => fsp.readFile(resultFile, { encoding: 'utf8' } )
        .then( result => {
          return fsp.remove(eventFile)
            .then(() => fsp.remove(resultFile))
            .then(() => {
