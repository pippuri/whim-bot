...
class StateMachine {
  ...
  isStateValid(tableName, oldState, newState) {
  ...
  logState(tableName, itemId, oldState, newState) {
    const payload = {
      id: lib.createId(),
      tableName: tableName,
      itemId: itemId,
  ...
  static changeState(tableName, itemId, oldState, newState) {
    ...
    if (!this.isStateValid(tableName, oldState, newState)) {
      return Promise.reject(new MaasError(`Invalid state(s) input. oldState: ${oldState}, newState ${newState}`, 400));
    }

    return this.logState(tableName, itemId, oldState, newState)
      .then(() => newState);
  }
}
