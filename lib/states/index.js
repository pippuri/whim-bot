 ...
 */
function logState(tableName, itemId, oldState, newState) {
  const payload = {
    id: lib.createId(),
    tableName: tableName,
    itemId: itemId,
 ...
 * RETURN: newState
 */
function changeState(tableName, itemId, oldState, newState) {
  ...
  return logState(tableName, itemId, oldState, newState)
    .then(() => newState);
}
