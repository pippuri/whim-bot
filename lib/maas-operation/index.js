...
function computeBalance(cost, profile) {
  ...
  console.log('computed balance', Math.round((balance - cost) * 100) / 100);

  if (balance > cost) {
    return Promise.resolve(Math.round((balance - cost) * 100) / 100);
  }
...
function updateBalance(identityId, newBalance) {
  ...
  console.log(`Update new balance ${newBalance} for identityId ${identityId}`);
