...
function containsAgency(agencyId) {
  return typeof tspData[agencyId] !== typeof undefined;
}
 ...
 */
function supportsAction(action, agencyId) {
  const method = actionMethodMap[action];

  // If no such method exist, return false
  if (typeof method === typeof undefined) {
    return false;
  }

  // Check if the TSP is actually found
  const tsp = tspData[agencyId];
  if (!tsp) {
    return false;
  }

  // Then check if there is a method mapping for it
  if (!tsp.adapter.endpoints[method]) {
    return false;
  }
...
function createBooking(booking) {
  ...
  // Create copy of the data, so that we won't mess up with it
  const copy = utils.cloneDeep(booking);

  return findAgency(copy.leg.agencyId)
    .then(tsp => {
      const options = {
        body: {
          leg: copy.leg,
      ...
      return [copy, invokeTSPAction(tsp, 'book', options)]; // Delegate booking call to specific TSP api endpoint
    })
...
function updateBooking(booking) {
  if (!booking || !booking.leg || !booking.leg.agencyId) {
    const message = 'updateBooking: Missing booking information; ' +
...
function cancelBooking(booking) {
  if (!booking || !booking.leg || !booking.leg.agencyId) {
    const message = 'cancelBooking: Missing booking information; ' +
      `booking: ${booking}, booking.leg: ${booking.leg}, booking.meta: ${booking.meta} ` +
      `booking.customer: ${booking.customer}, booking.customer.agencyId: ${booking.customer.agencyId}`;
    return Promise.reject(new Error(message));
  }
