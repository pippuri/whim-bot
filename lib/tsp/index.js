...
function createBooking(leg, profile, term, meta) {
  ...
  return changeBookingState(booking, 'PENDING')
    ...
    .then(found_tsp => {
      ...
      const url = found_tsp.adapter.baseUrl + found_tsp.adapter.endpoints.post.book;
      const options = Object.assign({
        json: true,
        body: booking,
      }, found_tsp.adapter.config);
    ...
    .spread((original_booking, altered_booking) => {
      ...
      if ( altered_booking.state === 'BOOKED' || altered_booking.state === 'NEW') {
        altered_booking.state = 'RESERVED';
      }
      return stateMachine.changeState('Booking', altered_booking.id, original_booking.state, altered_booking.state)
        .then(() => Promise.resolve(altered_booking));
    })
