...
const queryRequestSchema = require('maas-schemas/prebuilt/tsp/booking-options-list/request.json');
const reserveRequestSchema = require('maas-schemas/prebuilt/tsp/booking-create/request.json');
const retrieveRequestSchema = require('maas-schemas/prebuilt/tsp/booking-read-by-id/request.json');
const cancelRequestSchema = require('maas-schemas/prebuilt/tsp/booking-cancel/request.json');
...
class TransportServiceAdapter {
  ...
  static _toTSPFormat(booking) {
    const copy = utils.sanitize(booking);
   ...
   * @param {boolean} isRequest whether this is request (true) or response (false)
   * @param {request} the original request (for error reporting)
   * @return {Promise} resolving to the response, or reject with TSPError
   */
  _validate(schema, response, isRequest) {
    const type = isRequest ? 'request' : 'response';

    return validator.validate(schema, response)
      .catch(error => {
        console.warn(`[TSP] Caught a ${type} validation error: ${error.message}`);
        console.warn(error.stack);

        const message = `Transport Service Adapter: Invalid ${type}: ${error.message}`;
  ...
  query(params) {
    const query = params ? {
      ...
      fromName: params.fromName ? params.fromName : undefined,
      to: params.to ? params.to.join(',') : undefined,
      toName: params.toName ? params.toName : undefined,
    ...
    return this._validate(queryRequestSchema, query, true)
      .then(request => this._handleRequest('query', null, null, query))
      .then(response => this._validate(querySchema, response));
  }
   ...
   */
  reserve(booking) {
    return this._validate(reserveRequestSchema, booking, true)
      .then(request => this._handleRequest('reserve', null, booking, null))
      .then(response => this._validate(reserveSchema, response));
  }
   ...
   */
  retrieve(tspId) {
    return this._validate(retrieveRequestSchema, { tspId: tspId }, true)
      .then(request => this._handleRequest('retrieve', tspId, null, null))
      .then(response => this._validate(retrieveSchema, response));
  }
   ...
   */
  cancel(tspId) {
    return this._validate(cancelRequestSchema, { tspId: tspId }, true)
      .then(request => this._handleRequest('cancel', tspId, null, null))
      .then(response => this._validate(cancelSchema, response));
  }
  ...
  _handleRequestError(error) {
    ...
    switch (error.statusCode) {
      ...
      case 500:
        ...
        break;
      case 503:
        message = 'Temporary error communicating with TSP';
        break;
      default:
    ...
    return this._mapToMethod(operation)
      .then(method => {
        ...
        .then(response => {
          ...
          // Return sanified TSP output
          return utils.sanitize(response);
        }, error => {
