...
/**
 * Converts a decimal number to fixed precision.
 ...
 * @param {number} precision - The decimals to round to
 * @return {number} the decimal rounded number
 */
function toFixedImpl(number, precision) {
 ...
 *
 * Transformation will be called as follows:
 * transformation(value, context);
 *
 * @param {object} value - the object/array to visit
 * @param {function} transformation - the transformation function to execute
 * @param {object} context - optional context parameter, used in recursive calls
 * @see https://en.wikipedia.org/wiki/Visitor_pattern
 * @see https://en.wikipedia.org/wiki/Tree_traversal#In-order
 */
function transform(object, transformation, context) {

  // Handle the simple values (non-objects)
  if (typeof object !== 'object' || object === null) {
    return transformation(object, context);
  }
  ...
  if (Array.isArray(object)) {
    ...
    object.forEach(value => {
      const v = transform(value, transformation, object);
  ...
  Object.keys(object).forEach(key => {
    const v = transform(object[key], transformation, object);
    if (typeof v === typeof undefined) {
      return;
    }
    transformed[key] = v;
...
function removeNulls(input) {
  return transform(input, (value, context) => {
    if (value === null) {
...
function toFixed(input, digits) {
  return transform(input, value => {
    ...
    if (typeof input === 'number' && !Number.isInteger(input)) {
      return toFixedImpl(input, digits);
    }
