...
function transform(object, transformation) {
  ...
  if (Array.isArray(object)) {
    return object.map(value => transform(value, transformation));
  }

  // Handle a complex case (Object)
  const transformed = {};
  Object.keys(object).forEach(key => {
    transformed[key] = transform(object[key], transformation);
  });
...
function toFixed(input, digits) {
  ...
  // Handle a complex case (Array): Convert numbers in nested objects
  if (Array.isArray(input)) {
    return input.map(value => toFixed(value, digits));
  }
  ...
  Object.keys(input).forEach(key => {
    ...
    if (value !== null) {
      output[key] = toFixed(value, digits);
    }
  });
