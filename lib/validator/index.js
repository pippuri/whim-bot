...
function validate(schema, object, options) {
  const opts = Object.assign({ verbose: true, allErrors: true, validateSchema: false,
    addUsedSchema: false, meta: false, inlineRefs: false, sourceCode: false,
    errorDataPath: 'property', multipleOfPrecision: 6 }, options);
  ...
  return Promise.reject(ValidationError.fromValidatorErrors(ajvValidate.errors, object));
}
