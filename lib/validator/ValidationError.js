...
class ValidationError extends Error {
  ...
  static fromValidatorErrors(errors, object) {
    ...
    return new ValidationError(messages.join(''), object, errors);
  }
  ...
  static fromValue(dataPath, data, message, object) {
    const msg = `Validation error: '${dataPath}' ${trim(message)}, got '${JSON.stringify(data)}'`;
    const error = {
      dataPath,
      message,
      data,
    ...
    return new ValidationError(msg, object, [error]);
  }
}
