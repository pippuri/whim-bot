...
function chooseProviderByRegion(provider, from) {
  let subProvider = '';
  const regions = providerRegions[provider];
  if (regions) {
...
function resolveRoutesProvider(event, options) {
  return new Promise((resolve, reject) => {
    ...
    const subProvider = chooseProviderByRegion(provider, event.from);
    const functionName = 'MaaS-provider-' + provider + '-routes' + subProvider;
...
function getRoutes(serviceBus, identityId, parameters, options) {
  const routesPromise = resolveRoutesProvider(parameters, options)
