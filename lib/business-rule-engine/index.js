...
function getUserSpecificCostSpecs(context) {

  // TODO make freeForAllPlans configurable

  const freeForAllPlans = ['plan0'];

  // TODO read usersActivePlans from context

  const usersActivePlans = ['plan2'];

  const allPlans = freeForAllPlans.concat(usersActivePlans);
  const costSpecs = _.flatten(allPlans.map(planId => planDatabase[planId].costSpecs));
  return costSpecs;
}
...
function getApplicableSpecs(leg, costSpecs) {
...
function calculateLegTicketOptions(leg, costSpecs) {

  const applicableCostSpecs = getApplicableSpecs(leg, costSpecs);
  const ticketOptions = applicableCostSpecs.map(spec => createTicketOption(leg, spec));
...
function calculateItineraryCost(itinerary, costSpecs) {
  const ticketOptionsForLegs = itinerary.legs.map(leg => {
    return calculateLegTicketOptions(leg, costSpecs);
  });
...
function addLegCosts(routes, costSpecs) {
  for (var itinerary of routes.plan.itineraries) {
    itinerary.fare = {
      points: calculateItineraryCost(itinerary, costSpecs),
...
function getRoutes(serviceBus, identityId, parameters, options) {
  ...
  var costSpecsPromise = serviceBus.call('MaaS-profile-info', { identityId: identityId })
  .then(context => getUserSpecificCostSpecs(context));
  ...
  .then(all => new Promise((resolve, reject) => {
    const routes = all[0];
    const costSpecs = all[1];
    addLegCosts(routes, costSpecs);
