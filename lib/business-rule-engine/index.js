...
module.exports = {
  call: (ruleObject, options) => {
    ...
    if (ruleObject.rule === 'get-routes') {
      return serviceBus.call('MaaS-database-context-get', { principalId: ruleObject.userId })
      .then((context) => getPolicy(context.activePlans))
      .then((policy) => resolveRoutesProvider(ruleObject.parameters, ruleObject.options))
