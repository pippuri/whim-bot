...
class Database {
  ...
  static init(verbose) {
    ...
    if (_verbose) console.log(`Database::init,0: ${Database._handleCount}`);
  ...
  static cleanup(force) {
    if (_verbose) console.log(`Database::cleanup,0: ${Database._handleCount}`);
    ...
    // Update reference handle count
    Database._handleCount--;
    if (_verbose) console.log(`Database::cleanup,1: ${Database._handleCount}`);
    ...
    return Database._knex.destroy()
      .then(() => {
        ...
        if (_verbose) console.log(`Database::cleanup,2: ${Database._handleCount}`);
      });
  }
