...
class Provider extends Model {
  ...
  /**
   * Select a single provider using type or direct agencyId
   * @param providerType {String}
   * @param params {Object} contains agencyId {String - optional} and location
   */
  static selectProvider(providerType, params) {
    const table = Provider.tableName;

    if (params.hasOwnProperty('agencyId') && params.agencyId) {
      return Provider.raw(`
        SELECT "providerName", "providerType", "agencyId", "providerMeta", "providerPrio", "active"
        FROM "${table}"
        WHERE "agencyId"='${params.agencyId}'
        AND "active"=TRUE
        ORDER BY "providerPrio" ASC;
      `);
    }
    const location = params.location;
    return Provider.raw(`
      SELECT "providerName", "providerType", "agencyId", "providerMeta", "providerPrio", "active" FROM "${table}"
      WHERE ST_contains(
        the_geom,
        ST_GeomFromText(
  ...
  static insertProvider(providerName, providerType, agencyId, providerPrio, providerMeta, geometry) {
    const table = Provider.tableName;

    // eslint-disable-next-line max-len
    return Provider.raw(`
      INSERT INTO "${table}" ("providerName", "providerType", "agencyId", "providerPrio", "providerMeta", the_geom)
      VALUES ('${providerName}', '${providerType}', '${agencyId}', ${providerPrio}, '${providerMeta}', ST_GeomFromGeoJSON('${geometry}'));
    `);
  }
}
