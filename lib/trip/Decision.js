...
module.exports = class Decision {
  ...
  */
  scheduleLambdaTask(serviceName, event) {
    ...
    const flowInput = JSON.parse(JSON.stringify(this.flow.flowInput));
    flowInput.task = {
      name: serviceName,
      params: event,
    };
   ...
   * @param {Number} firedTime (Unix timestamp) when timer should fire
   * @param {String} taskName Enum task name as defined in TripWorkFlow
   * @param {Object} [taskParams] Additional parameters to be included into timer event
   */
  scheduleTimedTask(firedTime, taskName, taskParams) {
    ...
    // check if timer already scheduled
    const timers = this.flow.events.filter(event => {
      return (event.eventType === 'TimerStarted' &&
              event.timerStartedEventAttributes.timerId === decision.startTimerDecisionAttributes.timerId);
    });
    if (timers.length === 0) {
      ...
      if (taskParams) {
        flowInput.task.params = taskParams;
      }
      decision.startTimerDecisionAttributes.control = JSON.stringify(flowInput);
      this.decisions.push(decision);
    } else {
      console.warn(`[Decision] Warning, skipping schduling timed task as already scheduled; '${decision.startTimerDecisionAttributes.timerId}'`);
    }
  }
  ...
  closeFlow(result) {
    ...
    if (result) {
      decision.completeWorkflowExecutionDecisionAttributes.result = result;
    }
    this.decisions.push(decision);
  }
  ...
  get decisionTaskCompletedParams() {
    const params = JSON.parse(JSON.stringify(Decision_DecisionHash));
    params.taskToken = this.flow.taskToken;
    params.decisions = this.decisions;
    //console.info('Decision decisionTaskCompletedParams:', params);
    return params;
  }
