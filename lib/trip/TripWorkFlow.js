...
module.exports = class TripWorkFlow {
   ...
   */
  addTrip(trip) {
    if (!(trip instanceof Trip) || !trip.reference) {
      throw new MaaSError('Cannot create work flow without Trip with reference', 400);
    }
    this.trip = trip;
  ...
  get signalWorkflowExecutionParams() {
    ...
    console.log('TripWorkFlow signalWorkflowExecutionParamsParams:', tripFlowParams);
    return tripFlowParams;
  }
