...
function addLogPermission(arn, loggingFunctionName, stage) {
  ...
  return Promise.resolve()
    .then(() => Lambda.addPermission(params).promise())
    .catch(() => false);  // Fallback if the lambda doesn't exist
}
...
function removeLogPermission(arn, loggingFunctionName, stage) {
  ...
  return Promise.resolve()
    .then(() => Lambda.removePermission(params).promise())
    .catch(() => false); // Fallback if the lambda doesn't exist
}
...
function createSubscriptionFilter(destinationArn, filterName, filterPattern, logGroupName) {
  ...
  return Promise.resolve()
    .then(() => CloudWatchLogs.putSubscriptionFilter(params).promise())
    .then(data => {
...
function removeSubscriptionFilter(filterName, logGroupName) {
  ...
  return Promise.resolve()
    .then(() => CloudWatchLogs.deleteSubscriptionFilter(params).promise())
    .then(data => {
