...
/**
 * Returns a function which can be used in the catch clause of a promise chain
 */
 ...
 * This error handler also accepts a Database parameter which will be
 * automatically handled if an error is caught.
 */
function stdDatabaseErrorHandler(callback, event, Database, defaultErrorMessage, defaultErrorCode) {
...
function stdErrorHandler(callback, event, defaultErrorMessage, defaultErrorCode) {
  return _errorHandler(callback, event, undefined, defaultErrorMessage, defaultErrorCode, CAN_FAIL);
}
...
function alwaysSucceedErrorHandler(callback, event, defaultResponse, defaultResponseCode) {
  return _errorHandler(callback, event, undefined, defaultResponse, defaultResponseCode, ALWAYS_SUCCEED);
}
 ...
 * automatically handled if an error is caught.
 */
function alwaysSucceedDatabaseErrorHandler(callback, event, Database, defaultResponse, defaultResponseCode) {
  return _errorHandler(callback, event, Database, defaultResponse, defaultResponseCode, ALWAYS_SUCCEED);
}

module.exports = {
  stdErrorHandler,
  stdDatabaseErrorHandler,
  alwaysSucceedErrorHandler,
  alwaysSucceedDatabaseErrorHandler,
  MaaSError,
  BaseError,
};
