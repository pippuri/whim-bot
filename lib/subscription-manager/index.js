...
function updateUserCreditCard(userId, userInfo) {
  ...
  userId = encodeURIComponent(userId);
  return Request.post(`${API_URL}/customers/${userId}/credit_card`, { json: true, auth: auth, verbose: false, form: userInfo });
}
...
module.exports = {
  ...
  updateUserCreditCard: function (identityId, properties) {
    return updateUserCreditCard(identityId, convertToChargebee(properties)).then(user => {
      return formatUser(user);
    });
  },
