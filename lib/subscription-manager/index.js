...
function updatePlan(subId, updatedPlan, promoCode, prorate) {
  if (!subId) throw new Error('no user info');

  const postSubId = encodeURIComponent(subId);
  const form = { plan_id: updatedPlan, end_of_term: prorate ? false : true }; //eslint-disable-line no-unneeded-ternary
  if (promoCode) {
...
module.exports = {
  ...
  createUser: function (identityId, defaultPlanId, properties) {
    ...
    return createUser(identityId, defaultPlanId, convertToChargebee(chargebeeArgs))
  ...
  updatePlan: function (subscriptionId, planId, options) {
    return getUserSubscription(subscriptionId).then( user => {
      ...
      if (user.subscription) {
        return updatePlan(subscriptionId, planId, opts.promoCode, opts.prorate);
      }
