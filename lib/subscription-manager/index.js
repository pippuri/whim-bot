...
function deleteUserSubscription(userId) {
  userId = encodeURIComponent(userId);
  return Request.post(`${API_URL}/customer/${userId}/delete`, { json: true, auth: auth });
}
...
function updatePlan(subId, updatedPlan, promoCode, prorate, changeTerm) {
  ...
  return Request.post(`${API_URL}/subscriptions/${postSubId}`, { verbose: true, json: false, auth: auth, form: form })
     .then( response => JSON.parse(response.toString()));
}
