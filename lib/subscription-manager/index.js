...
function updateUserCreditCard(userId, userInfo) {
  ...
  userId = encodeURIComponent(userId);
  console.log('Updating Chargebee payment info', userInfo)
  const ret = Request.post(`${API_URL}/customers/${userId}/credit_card`, { json: false, auth: auth, verbose: true, form: userInfo });
  return ret;
}
...
function convertToChargebee(properties) {
  ...
  // debug info
  console.log('billing properties', JSON.stringify(properties, null, 2)) 
  ...
  // debug info
  console.log('billing post-conversion', JSON.stringify(ret, null, 2)) 
  return ret;
}
