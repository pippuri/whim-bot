...
var PRODUCT_API_KEY = 'test_1C7nCxniaFbsqOoOcubv9N22LXogN98DL:';
var AUTH_HEADERS =  {
...
function findUser(userId) {
  return request.get(PRODUCT_BASE_URL + CUSTOMERS_ENDPOINT + '/' + userId, {
    json: true,
    headers: AUTH_HEADERS,
  });
}
...
function getUserSubscription(userId) {
  return request.get(PRODUCT_BASE_URL + SUBSCRIPTIONS_ENDPOINT + '/' + userId, {
    json: true,
    headers: AUTH_HEADERS,
  });
}
...
function createUser(userId, userInfo) {
  ...
  return request.post(PRODUCT_BASE_URL + CUSTOMERS_ENDPOINT + '/' + userId, {
    json: false,
    headers: AUTH_HEADERS,
    body: userInfo,
  });
...
function updateUserBilling(userId, userInfo) {
  ...
  if (!userInfo.hasOwnProperty('billing_address[country]')) throw new Error('no user country');
  if (!userInfo.hasOwnProperty('billing_address[zip]')) throw new Error('no user ZIP code');
  return request.post(PRODUCT_BASE_URL + CUSTOMERS_ENDPOINT + '/' + userId + '/' + BILLING_INFO_ENDPOINT, {
    json: false,
    headers: AUTH_HEADERS,
    body: userInfo,
  });
...
function updateUserCreditCard(userId, userInfo) {
  ...
  return request.post(PRODUCT_BASE_URL + CUSTOMERS_ENDPOINT + '/' + userId + '/' + CARD_ENDPOINT, {
    json: false,
    headers: AUTH_HEADERS,
    body: userInfo,
  });
...
module.exports = {
  ...
  getUserPlans: function (principalId) {
    return getUserPlans(principalId);
  },
