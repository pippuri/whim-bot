...
// Get available plans
// NOTE: If a `limit` parameter is not specified, it will default to 10.
//       The limit is thus set to the maximum allowed value: 100.
function getStorePlans() {
  return Request.get(API_URL + '/plans', { json: true, auth: auth, limit: 100 });
}
...
function getStoreAddons() {
  return Request.get(`${API_URL}/addons`, { json: true, auth: auth, limit: 100 });
}
...
function createUserSubscription(userId, planId, userInfo) {
  if (!userInfo) throw new MaaSError('no user info', 400);
  if (!planId) throw new MaaSError('no default plan info', 400);
  if (!userId) throw new MaaSError('no User ID info', 400);
  if (!userInfo.phone) throw new MaaSError('no user phone', 400);
...
function updateUser(userId, userInfo) {
  ...
  // these are ChargeBee types
  if (!userInfo.first_name) throw new MaaSError('no user first_name', 400);
  if (!userInfo.last_name) throw new MaaSError('no user last_name', 400);
  if (!userInfo.email) throw new MaaSError('no user email', 400);
  if (!userInfo.phone) throw new MaaSError('no user phone', 400);
  if (userInfo.hasOwnProperty('billing_country')) {
    userInfo['billing_address[country]'] = userInfo.billing_country;
    delete userInfo.billing_country;
...
function updatePlan(subId, updatedPlan, promoCode, prorate, changeTerm) {
  if (!subId) throw new MaaSError('no user info', 400);
...
function makePurchase(userId, productId, amount) {
  if (!userId) throw new MaaSError('no user info', 400);
...
function getLoginURL(identityId) {
  if (!identityId) throw new MaaSError('no user info', 400);
...
function formatUser(user) {
  ...
  if (!user) {
    throw new MaaSError('User info not found', 500);
  }
  ...
  getLoginURL: function (identityId) {
    return getLoginURL(identityId).then( response => {
      if (!response.hasOwnProperty('portal_session')) {
        throw new MaaSError('Portal Session not created', 500);
      }
      return {
