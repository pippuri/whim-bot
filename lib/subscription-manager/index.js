...
function makePurchase(userId, amount, title) {
  if (!userId) throw new Error('no user info');

  return Request.post(`${API_URL}/invoices/charge`, { json: false, auth: auth, form: { customer_id: userId, amount: amount, description: title } });
}
...
function convertToChargebee(properties) {
  ...
  if (properties.phone) ret.phone = properties.phone;
...
module.exports = {
  ...
  createUser: function (identityId, defaultPlanId, properties) {
    ...
    return createUser(identityId, defaultPlanId, convertToChargebee(properties)).then(user => {
      return formatUser(user);
    });
  },
  ...
  makePurchase: function (identityId, amount, title) {
    return makePurchase(identityId, amount, title);
  },
