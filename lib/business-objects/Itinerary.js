...
class Itinerary {
  ...
  static create(itineraryInput, identityId, transaction) {
    ...
    return validator.validate(awsUnitsSchema.definitions.identityId, identityId)
      ...
      .then(itinerary => {
        ...
        return Promise.all(itinerary.itinerary.legs.map(legInput => {
      ...
      .then(itinerary => {
        transaction.meta(models.Itinerary.tableName, itinerary.itinerary.id);
        return transaction.bind(models.Itinerary)
          .query()
   ...
   */
  static query(identityId, startTime, endTime, states, bookingId) {
    ...
    if (bookingId === undefined) {
      ...
      query = query
        .select('Itinerary.*')
        .join('Leg', 'Leg.itineraryId', 'Itinerary.id')
        .where('Leg.bookingId', bookingId)
        .andWhere('identityId', identityId);
    }

    if (startTime !== null) {
      query = query.andWhere('Itinerary.startTime', '>=', (new Date(startTime)).toISOString());
    }

    if (endTime !== null) {
      query = query.andWhere('Itinerary.endTime', '<=', (new Date(endTime)).toISOString());
    }

    if (states.length > 0) {
      query = query.whereIn('Itinerary.state', states);
    }
  ...
  /**
   * Cancels the Itinerary. If the itinerary is already cancelled, the
   * cancellation will be automatically succesful.
   * If the itinerary is "active", cancel operation is tried and result is
   * always successful either with a state of CANCELLED or
   * CANCELLED_WITH_ERRORS.
   * If itinerary is not active (e.g. FINISHED), cancellation fails and
   * the state is kept as it was.
   *
   * If input transaction is given, all database operations here are bound to
   * this transaction. Whoever calls this create-method, must then commit or
   * rollback the transaction! The transaction must be created with
   * Itinerary.startTransaction() by caller.
   *
   * @param {object} transaction - Objection transaction object
   * @return {Promise -> Itinerary}
   */
  cancel(transaction) {
    ...
    const paidFare = (this.itinerary.fare && this.itinerary.fare.points) || 0;
    ...
    return Promise.resolve()
      .then(() => {
        const legCancels = this.itinerary.legs.map(leg => {
          ...
          return leg.cancel(transaction).reflect();
        });
      ...
      .then(() => {
        ...
        return Profile.increaseBalance(this.itinerary.identityId, refund, transaction);
      })
      // persist
      .then(() => {
        transaction.meta(models.Itinerary.tableName, this.itinerary.id);
        return transaction.bind(models.Itinerary)
          .query()
      ...
      .catch(err => {
        ...
        return transaction.rollback()
          .catch(() => null)
          .then(() => Promise.reject(new Error(`Itinerary ${this.itinerary.id} cancel failed: ` + err)));
      });
  }
   ...
   *
   * If input transaction is given, all database operations here
   * are bound to this transaction. Whoever calls this pay-method, must then commit or rollback the transaction!
  ...
  pay(transaction) {
    ...
    return Profile.retrieve(this.itinerary.identityId)
      .then(profile => {
        ...
        console.info(`[Itinerary] Purchasing ${fare} points`);
        return Profile.decreaseBalance(this.itinerary.identityId, fare, transaction);
      })
  ...
  finish(transaction) {
    ...
    const legFinishs = this.itinerary.legs.map(leg => {
      return leg.finish(transaction).reflect();
    });
  ...
  fromName() {
    return this._extractName().fromName;
  }

  toName() {
    return this._extractName().toName;
  }

  /**
   * Extract itinerary fromName and toName
   */
  _extractName() {
    const firstLeg = this.itinerary.legs[0].toObject();
    const lastLeg = this.itinerary.legs[this.itinerary.legs.length - 1].toObject();
    return {
      fromName: firstLeg.from.name ? firstLeg.from.name : 'Origin',
      toName: lastLeg.to.name ? lastLeg.to.name : 'Destination',
    };
  }
