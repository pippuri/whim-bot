  ...
  cancel(transaction) {
    ...
    if (newTransaction) {
      action = trx.start()
        ...
        .then(() => trx.meta(models.Itinerary.tableName, this.itinerary.id));
    } else {
    ...
    return action
      ...
      .then(updateCount => {
        ...
        if (newTransaction) {
          const message = `Trip from ${this.itinerary.fromName()} to ${this.itinerary.toName()} cancelled and refunded`;
