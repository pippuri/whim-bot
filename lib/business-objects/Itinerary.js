...
class Itinerary {
  ...
  static create(itineraryInput, identityId) {
    return validator.validate(awsUnitsSchema.definitions.identityId, identityId)
      ...
      .then(itinerary => {
        return objection.transaction(models.Itinerary, Itinerary => {
          return Itinerary
            .query()
    ...
    return objection.transaction.start(models.Itinerary)
      ...
      .then(() => {
        const legCancels = this.itinerary.legs.map(leg => {
          ...
          // into transaction, as we cannot control TSP actions as atomic.
        ...
        if (this.state === 'CANCELLED') {
          ...
          return Profile.retrieve(this.itinerary.identityId)
            .then(profile => {
              ...
              return Profile.update(this.itinerary.identityId, { balance: newBalance })
                .then(() => {
                  refunded = true;
                  return Promise.resolve();
                });
      ...
      // handle errors
      .catch(err => {
        console.error('[Itinerary] failed cancel itinerary, err:', err.stack || err);
        const rollback = trx ? trx.rollback() : Promise.resolve();
        return rollback
          .then(() => {
            if (refunded === true && oldBalance) {
              console.warn(`[Itinerary] Restoring original ${oldBalance} points for customer...`);
              return Profile.update(this.itinerary.itineraryId, { balance: oldBalance });
            }
            return Promise.resolve();
          })
        ...
        return rollback
          .then(() => {
            if (paid === true && oldBalance) {
              console.warn(`[Itinerary] Restoring original ${oldBalance} points for customer...`);
              return Profile.update(this.itinerary.identityId, { balance: oldBalance });
            }
            return Promise.resolve();
          })
