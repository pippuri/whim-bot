...
class Itinerary {
  ...
  /**
   * Extract itinerary fromName
   */
  fromName() {
    const firstLeg = this.itinerary.legs[0];
    return firstLeg.from.name ? firstLeg.from.name : 'Origin';
  }

  /**
   * Extract itinerary toName
   */
  toName() {
    const lastLeg = this.itinerary.legs[this.itinerary.legs.length - 1];
    return lastLeg.to.name ? lastLeg.to.name : 'Destination';
  }

  /**
   * Extract itinerary short toName
   */
  shortToName() {
    const lastLeg = this.itinerary.legs[this.itinerary.legs.length - 1];
    const name = lastLeg.to.name ? lastLeg.to.name : 'Destination';
    return name.split(',')[0];
  }
