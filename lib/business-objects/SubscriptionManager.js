...
class SubscriptionManager {
  ...
  static createCustomer(customer) {
    ...
    const paymentAndCard = (customer.paymentMethod) ?
      SubscriptionManager.toChargebeePayment(customer) : undefined;
    const payload = Object.assign({}, paymentAndCard, {
  ...
  static updateCustomer(customer) {
    ...
    if (customer.paymentMethod && customer.paymentMethod.type !== 'card') {
      const method = SubscriptionManager.toChargebeePayment(customer);
      const paymentForm = SubscriptionManager.encodeToChargebeeFormat(method);
      paymentPromise = request.post(
        `${API_URL}/customers/${customer.identityId}/update_payment_method`,
        { auth: REQUEST_AUTH, form: paymentForm }
      )
      .catch(SubscriptionManager.parseError)
      .then(response => JSON.parse(response.toString()));
    } else if (customer.paymentMethod && customer.paymentMethod.type === 'card') {
    ...
    return Promise.all([billingPromise, paymentPromise])
      .then(responses => {
  ...
  static toChargebeeAddress(customer) {
    ...
    /**
     * Converts a user Profile to Chargebee compatible payment method format.
     *
     * @param {object} customer - the user/customer in our own customer schema
     * @return {object} Customer payment method in Chargebee format
     */
  static toChargebeePayment(customer) {
    if (typeof customer !== 'object') {
      throw new TypeError(`'customer' must be an object, got '${customer}'`);
    }
    ...
    if (method && method.type === 'stripe') {
      ...
      return {
        type: 'card',
        gateway: 'stripe',
        tmp_token: method.token,
      };
    } else if (method && method.type === 'card') {
      // Direct card (for testing purposes only)
      return {
        type: 'card',
        gateway: 'chargebee',
      };
  ...
  static fromChargebeeEstimate(estimate) {
    ...
    const taxesTotal = estimate.taxes.reduce((sum, tax) => sum + tax.tax_amount, 0);
    pricing.total = {
