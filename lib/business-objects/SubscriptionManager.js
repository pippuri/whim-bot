...
// Static private cache with 5 minute life-time
const cache = lru({ max: 100, maxAge: 1000 * 60 * 5 });
...
class SubscriptionManager {
  ...
  static get DEFAULT_PAYMENT_METHOD() {
    return { type: 'unknown',  valid: false };
  }

  static get DEFAULT_PLAN_ID() {
    return 'fi-whim-payg';
  }

  static get TOPUP_ID() {
    return 'fi-whim-top-up';
  }
  ...
  static createCustomer(customer) {
    console.info(`[SubscriptionManager] Creating customer ${JSON.stringify(customer)}`);
    ...
    try {
      ...
      const payload = Object.assign(SubscriptionManager.toChargebeeCustomer(customer), {
    ...
    return request.post(url, { auth: REQUEST_AUTH, form: form })
      .catch(SubscriptionManager.toMaaSError)
      .then(response => {
   ...
   * @return {Promise<object|Error>} The full customer data
   */
  static retrieveCustomer(customerId) {
    console.info(`[SubscriptionManager] Retrieving customer ${customerId}`);
  ...
  static updateCustomer(customer) {
    console.info(`[SubscriptionManager] Updating customer ${JSON.stringify(customer)}`);
    ...
    try {
      ...
      const customerForm = SubscriptionManager.encodeToChargebeeFormat(SubscriptionManager.toChargebeeCustomer(customer));
      promises.push(request.post(
        `${API_URL}/customers/${customer.identityId}`,
        { auth: REQUEST_AUTH, form: customerForm }
      ));
    ...
    return Promise.all(promises)
      ...
      .then(responses => {
        const billingAddress = responses[1].customer.billing_address;
        let paymentMethod;
        let card;

        if (typeof responses[2] !== 'undefined') {
          paymentMethod = responses[2].customer.payment_method;
          card = responses[2].card;
        }
  ...
  static deleteCustomer(customerId) {
    console.info(`[SubscriptionManager] Deleting customer ${customerId}`);
  ...
  static updateUser(user) {
    console.info(`[SubscriptionManager] Updating user (shipping address) to ${JSON.stringify(user)}`);
    ...
    return request.post(url, { auth: REQUEST_AUTH, form: form })
      ...
      .then(response => {
        const json = JSON.parse(response.toString());
        SubscriptionManager.fromChargebeeAddress(json.subscription.shipping_address, user.identityId);
      });
  }
  ...
  static createSubscription(subscription, customerId, userId) {
    console.info(`[SubscriptionManager] Creating subscription ${userId} with ${JSON.stringify(subscription)}`);
  ...
  /**
   * Retrieves a user's subscription for the current or the next period.
   *
   * @param {string} userId - the user identity to fetch the subscription for
   * @param {boolean} [nextPeriod=false] - retrieve the next period
   * @return {object} a single subscription object
   */
  static retrieveSubscriptionByUserId(userId, nextPeriod = false) {
    console.info(`[SubscriptionManager] Retrieving subscription ${userId}, nextPeriod=${nextPeriod}`);

    // We always need to call Chargebee to check if there are scheduled changes (that we can retrieve)
    // based on that, we can return the current or future subscription.
    const url = `${API_URL}/subscriptions/${userId}`;
    return request.get(url, { auth: REQUEST_AUTH })
      .catch(SubscriptionManager.toMaaSError)
      .then(response => {
        // If there are no scheduled changes, assume the future subscription is the same
        // as the following one.
        const json = JSON.parse(response.toString());
        if (!nextPeriod || !json.subscription.has_scheduled_changes) {
          return response;
        }

        // Fetch the scheduled changes (e.g. in next period, the subscription will be this)
        const url = `${API_URL}/subscriptions/${userId}/retrieve_with_scheduled_changes`;
        return request.get(url, { auth: REQUEST_AUTH });
      })
      .then(response => {
        const json = JSON.parse(response.toString());
  ...
  static retrieveSubscriptionsByCustomerId(customerId) {
    console.info(`[SubscriptionManager] Retrieving subscriptions of ${customerId}`);
   ...
   * @return {Promise<object|ValidationError>} The new subscription data
   */
  static updateSubscription(subscription, customerId, userId, immediate = true, replace = false) {
    console.info(`[SubscriptionManager] Updating subscription ${userId} to ${JSON.stringify(subscription)}`);
    ...
    const annotations = {
      ...
      // We do not replace coupons to avoid customers losing money when switching plans
      //replace_coupon_list: replace,
      end_of_term: !immediate,
      force_term_reset: immediate && replace,
    };
    ...
    return request.post(url, { auth: REQUEST_AUTH, form: form })
      ...
      .then(response => {
        ...
        // Reactivate subscription if needed (specified a plan change &
        // subscription is in scheduled cancellation or cancellation mode).
        // We catch all the errors, because we do not want the subscription
        // change fail because of these (the user is already charged).
        const status = json.subscription.status;
        const hasPlanChanges = typeof subscription.plan === 'object';

        // Try to renew the subscription if possible. Warn on error.
        let promise;
        if (status === 'non_renewing' && hasPlanChanges) {
          const url = `${API_URL}/subscriptions/${userId}/remove_scheduled_cancellation`;
          promise = request.post(url, { auth: REQUEST_AUTH });
        } else if (status === 'cancelled' && hasPlanChanges) {
          const url = `${API_URL}/subscriptions/${userId}/reactivate`;
          promise = request.post(url, { auth: REQUEST_AUTH });
        } else {
          promise = Promise.resolve();
        }

        // In any case - success or failure, return the subscription.
        return promise.catch(error => {
          console.warn(`Error: Subscription reactivation failed: ${error.message}, ${error.response}`);
          console.warn(error.stack);
        })
        .then(() => json.subscription);
      })
      .then(subscription => {
        return SubscriptionManager.fromChargebeeSubscription(subscription);
      });
  }
  ...
  static deleteSubscription(userId) {
    console.info(`[SubscriptionManager] Deleting subscription ${userId}`);
   ...
   * @param {boolean} [replace=false] - whether to do a full replacement, or add to the existing subscription
  ...
  static estimateSubscriptionUpdate(subscription, customerId, userId, immediate, replace) {
    console.info(`[SubscriptionManager] Estimating subscription ${userId}, update with ${JSON.stringify(subscription)}, replace=${replace}`);
   ...
   * @return {object} itemized subscription costs
   */
  static estimateNewSubscription(subscription, country, zipCode) {
    console.info(`[SubscriptionManager] Estimating new subscription ${JSON.stringify(subscription)}; country=${country}, zipCode=${zipCode}`);

    let form;
    let formString;
    try {
      // Perform the validation of the object - let TypeError happen if need be.
      subscription = validator.validateSync(subscriptionOptionSchema, subscription);
      country = validator.validateSync(
        addressSchema.definitions.country, country);
      ...
      // Do a cache lookup from the form values to avoid extra Chargebee calls
      formString = JSON.stringify(form);
      const cachedValue = cache.get(formString);
      if (cachedValue) {
        //console.info('[SubscriptionManager] estimateNewSubscription cache hit');
        return Promise.resolve(cachedValue);
      }
      //console.info('[SubscriptionManager] estimateNewSubscription cache miss');
    } catch (error) {
      return Promise.reject(error);
    }
    ...
    return request.post(url, { auth: REQUEST_AUTH, form: form })
      ...
      .then(response => {
        ...
        // Note: Chargebee claims that there's no invoice estimate if no
        // immediate invoice will be generated. However, we get one for
        // zero-priced plans, but not always
        const estimate = json.estimate.invoice_estimate
          || json.estimate.next_invoice_estimate || { total: 0 };
        const reformatted = SubscriptionManager.fromChargebeeEstimate(estimate);

        // Save the value to cache
        cache.set(formString, reformatted);
        return reformatted;
      });
  }
  ...
  static findSubscriptionOptions(location) {
    console.info(`[SubscriptionManager] Finding subscription options at ${JSON.stringify(location)}`);

    // TODO Implement lat-lon filtering and geolocation lookup when we expand
    const country = 'FI';
    const zipCode = '00100';

    // Fetch estimates for each plan and annotate the itemized prices if we don't
    // have a fixed unit price already
    const promises = subscriptionOptions.map(option => {
      return Promise.resolve()
        .then(() => SubscriptionManager.estimateNewSubscription(option, country, zipCode))
        .then(estimate => {
          const annotated = utils.cloneDeep(option);
          const items = estimate.lineItems;
          const discounts = estimate.discounts;

          // Remove the parts that are implementation specific
          delete annotated.availability;
    ...
    return Promise.filter(promises, inspection => {
      if (inspection.isFulfilled()) {
        return true;
      }
      console.error('findSubscriptionOptions failed with', inspection.reason());
      return false;
    })
   ...
   */
  static findRawSubscriptionOption(planId) {
    ...
    try {
      ...
      return Promise.resolve(utils.cloneDeep(option));
    } catch (error) {
      return Promise.reject(error);
    }
  }
   ...
   * for internal lookups, and hence does not price the plans.
   *
   * @param {object} subscription - the subscription to annotate
   * @param {string} [planId] - the plan id (to lookup the subscription as a reference)
   */
  static annotateSubscription(subscription, planId) {
    // Defaults
    planId = planId || (subscription.plan ? subscription.plan.id : undefined);

    if (!planId) {
      const message = 'Cannot annotate subscription, missing planId';
      return Promise.reject(new TypeError(message));
    }

    return SubscriptionManager.findRawSubscriptionOption(planId)
      .then(option => {
        const subsCopy = utils.cloneDeep(subscription);
        const annotated = option;

        // Annotate the plan
        annotated.plan = option.plan;
        annotated.terms = subsCopy.terms;

        // Annotate the add-ons
        annotated.addons = (subsCopy.addons || []).map(addon => {
          const optAddon = (option.addons || []).find(a => a.id === addon.id);
          if (optAddon) {
            return Object.assign({}, optAddon, addon);
          }

            // No addon found from plan, no info available (error case)
          console.warn(`Plan/addon mismatch: Could not annotate addon '${addon.id}' for plan '${planId}'`);
          return addon;
        });

        // Annotate the coupons
        annotated.coupons = (subsCopy.coupons || []).map(coupon => {
          const optCoupon = (option.coupons || []).find(c => c.id === coupon.id);
          if (optCoupon) {
            return Object.assign({}, optCoupon, coupon);
          }

            // No addon found from plan, no info available (error case)
          console.warn(`Plan/coupon mismatch: Could not annotate coupon '${coupon.id}' for plan '${planId}'`);
          return coupon;
        });
   ...
   * @param {string} [planId] - the plan id (to lookup the subscription as a reference)
   */
  static annotateSubscriptionWithPrices(subscription, planId) {
    // Defaults
    planId = planId || (subscription.plan ? subscription.plan.id : undefined);
    ...
    return SubscriptionManager.findRawSubscriptionOption(planId)
      .then(option => {
        const annotated = utils.cloneDeep(subscription);

        // Annotate the plan
        if (typeof option === 'undefined') {
          console.warn(`Plan/option mismatch: Could find subscription option for '${planId}'`);
          return annotated;
        }

        if (annotated.plan) {
          Object.assign(annotated.plan, { price: option.plan.price });
        }

        // Annotate the add-ons
        annotated.addons.forEach(addon => {
          const optAddon = (option.addons || []).find(a => a.id === addon.id);
          if (optAddon && optAddon.unitPrice) {
            Object.assign(addon, { unitPrice: optAddon.unitPrice });
            return;
          }
  ...
  static toChargebeeSubscription(subscription, customerId, userId) {
    ...
    if (subscription.plan) {
      ...
      // Support reduced unit prices
      if (subscription.plan.price) {
        response.plan_unit_price = 100 * subscription.plan.price.amount;
        response.currency_code = subscription.plan.price.unit;
      }
    }
    ...
    if (subscription.addons) {
      // Map addons to Chargebee format & filter away the ones where quantity < 1
      response.addons = subscription.addons.map(addon => {
        const modified =  { id: addon.id, quantity: addon.quantity };

        // Support reduced unit prices; Ensure the currency units match
        if (addon.unitPrice) {
          modified.unit_price = 100 * addon.unitPrice.amount;

          if (!response.currency_code) {
            response.currency_code = addon.unitPrice.unit;
          } else if (response.currency_code !== addon.unitPrice.unit) {
            throw new TypeError('Plans and add-ons must all be priced with the same currency');
          }
        }
  ...
  /**
   * Converts a customer Contact to Chargebee customer format (name, e-mail, phone).
   *
   * @param {object} customer - the user/customer in our own customer schema
   * @return {object} Customer in Chargebee specific format
   */
  static toChargebeeCustomer(customer) {
  ...
  /**
   * Converts a user Contact to Chargebee compatible shipping address format
   * Note that we currently do not store user street address in the request,
   * so the profile is at best partial.
   *
   * @param {object} customer - the user/customer in our own customer schema
   * @return {object} Address in Chargebee specific format
   */
  ...
  static toChargebeeCard(customer) {
    ...
    ['city', 'country', 'zipCode'].forEach(attribute => {
   ...
   * @param {object} [card] - Chargebee card information
   * @return {object} customer object that follows MaaS customer schema
   */
  static fromChargebeeAddress(address, identityId, paymentMethod, card) {
    ...
    response.paymentMethod =
      SubscriptionManager.fromChargebeePaymentMethod(paymentMethod, card);

    return response;
  }

  /**
   * Parses subscription object from subscription option (or its subset)
   *
   * @param {object} - option subscription option (event payload)
   * @return {object} subscription that is parsed from the option
   */
  static fromSubscriptionOption(option) {
    const subscription = {
  ...
  static fromChargebeeSubscription(subscription) {
    const response = {
      ...
      terms: {
        activated: subscription.activated_at * 1000,
        modified: subscription.updated_at * 1000,
        validity: {
          startTime: subscription.current_term_start * 1000,
          endTime: subscription.current_term_end * 1000,
        },
        scheduledChanges: subscription.has_scheduled_changes,
      },
    };

    // Reflect the MaaS subscription plan id from Chargebee plan state & id
    // Cancelled plans should fallback to the default plan
    if (subscription.plan_id) {
      if (subscription.status === 'cancelled') {
        response.plan = { id: SubscriptionManager.DEFAULT_PLAN_ID };
      } else {
  ...
  static fromChargebeeEstimate(estimate) {
    ...
    pricing.lineItems = lineItems.map(item => {
      ...
      // Note: Chargebee computes taxes from the discounted price, not the original one
      const unitPrice = item.unit_amount;
      ...
      return {
        ...
        quantity: quantity,
        unitPrice: {
          amount: utils.toFixed(0.01 * unitPrice, 2),
          discount: utils.toFixed(0.01 * unitDiscount, 2),
    ...
    // One year from now
    const MAX_DATE_PROMISED = Date.now() + 1000 * 3600 * 24 * 365;
    const starts = lineItems.reduce((start, i) => Math.max(i.date_from * 1000, start), Date.now());
    const ends = lineItems.reduce((end, i) => Math.min(i.date_to * 1000, end), MAX_DATE_PROMISED);
    pricing.terms = {
      validity: {
        // Use the latest start time of the line items for start time,
        // earliest finish time for end time
        startTime: starts,
        endTime: ends,
      },
    };
  ...
  /**
   * Parses a card object from a Chargebee JSON webhook.
   *
   * @param {object} paymentMethod - Chargebee payment_method to parse
   * @param {object} card - Chargebee card to parse
   * @return {object} paymentMethod object that follows our paymentMethod schema
   */
  static fromChargebeePaymentMethod(chargebeePaymentMethod, chargebeeCard) {
    const method = chargebeePaymentMethod || {};
    const card = chargebeeCard || {};

    // Helper to extract the card issuer
    const _cardIssuer = card => {
      if (card.card_type && card.card_type.length && card.card_type.length > 0) {
        return card.card_type;
      }
      return null;
    };

    // Helper to convert yy/mm card expiry date to a timestamp
    const _cardExpiryTimestamp = card => {
      if (card.expiry_year > 0 && card.expiry_month > 0) {
        // Card expires at the end of yy/mm, so add 1 to month
        // (and then subtract 1 due to JS Date semantics)
        const d = new Date(card.expiry_year, card.expiry_month);

        // Return timestamp
        return d.getTime();
      }

      // No card information, so return the default invalid value
      return 0;
    };
    ...
    const response = {
      ...
      expiry: _cardExpiryTimestamp(card),
      issuer: _cardIssuer(card),
      valid: false,
    };

    if (method.gateway === 'stripe') {
      response.type = 'stripe';
    }

    // Set the valid flag based on the paymentMethod status.
    // card_deleted and card_expired events will have an empty paymentMethod
    // so valid flag will default to false, and issuer will be deleted
   ...
   */
  static toMaaSError(error) {
    // Parse the errors with HTTP semantics
    let message = error.message;
    let code = error.statusCode || 500;

    // Recast all 401s to 403 to prevent mixing them to custom authorizer errors
    if (code === 401) {
      code = 403;
    }
    ...
    if (error instanceof request.HTTPError) {
      ...
      const errorParam = response.error_param;
      ...
      switch (response.api_error_code) {
        ...
        case 'resource_not_found':
          if (/^coupon_ids\[(\d+)\]$/.test(errorParam)) {
            message = 'Invalid coupon given';
            code = 400;
            break;
          }
          if (/^addons\[id\]\[(\d+)\]$/.test(errorParam)) {
            message = 'Invalid addon given';
            code = 400;
            break;
          }
          if ('subscription[plan_id]' === errorParam) {
            message = 'Invalid plan given';
            code = 400;
            break;
          }
