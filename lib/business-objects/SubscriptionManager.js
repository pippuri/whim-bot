  ...
  static updateSubscription(subscription, customerId, userId, immediate, replace) {
    ...
    return request.post(url, { auth: REQUEST_AUTH, form: form })
      ...
      .then(response => {
        ...
        // subscription is in schedulled cancellation or cancellation mode).
        ...
        if (status === 'non_renewing' && hasPlanChanges) {
          ...
          promise = Promise.resolve(json.subscription);
        }

        return promise.catch(error => {
          console.warn(`Subscription reactivation failed: ${error.message}`);
