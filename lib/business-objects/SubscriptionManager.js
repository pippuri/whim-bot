...
class SubscriptionManager {
  ...
  static createCustomer(customer) {
    ...
    return Promise.resolve()
      .then(() => {
        ...
        } else if (method && method.type === 'stripe') {
          payload.payment_method = SubscriptionManager.toChargebeePayment(customer);
        }
  ...
  static fromChargebeeEstimate(estimate) {
    ...
    const taxesTotal = estimate.taxes.reduce((sum, tax) => sum + tax.amount, 0);
    pricing.total = {
