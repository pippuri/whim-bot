 ...
 * transaction log. Additionally, it provides global user level mutual
 * exclusivity (mutex). Only one transaction per user can be ongoing
 * simulatanously. Postgresql Advisory Locks, bind to the transaction,
 * is used for the purpose.
 *
 * To get the transactions properly logged, one should call
...
class Transaction {
  ...
  constructor(identityId) {
    ...
    // TODO: to verify does this algorithm provide unique lock id per user
    this._lockId = parseInt(identityId.replace(/-/g, '').substr(identityId.indexOf(':') + 1, 13), 16);
  }
  ...
  start(level) {
    ...
    console.info(`[Transaction start] New transaction id ${this._id}, isolation level: ${level} ${this._identityId}, lock id: ${this._lockId}`);
    return objection.transaction.start(models.Database.knex)
      ...
      .then(() => this._transaction.raw(`SELECT pg_advisory_xact_lock(${this._lockId})`))
  ...
  */
  ...
  commit(message) {
    ...
    try {
      ...
      if (typeof this._message !== 'string' || this._message.length <= 3) {
  ...
  /**
   * Set the message of the transaction. If message is set, commit will write transaction with this
   * message into the transaction log.
   * @param {String} message
   */
  set message(message) {
    if (typeof message !== 'string' || message === '' || message.length > Transaction.MAX_MESSAGE_LENGTH) {
      throw new Error(`Transaction message must be non-empty string and shorter than ${Transaction.MAX_MESSAGE_LENGTH} characters.`);
    }
