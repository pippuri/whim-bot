...
class Transaction {
  ...
  */
  associate(key, value) {
    ...
    if (typeof key !== 'string' && isNaN(key)) return Promise.reject(new TypeError('Association key must be String or Number'));
    if (!value) return Promise.reject(new TypeError('Cannot associate falsy value'));

    if (!Array.isArray(this.associations[key])) {
      this.associations[key] = [value];
    } else if (!this.associations[key].some(_value => _value === value)) {
   ...
   */
  rollback(message) {
    if (typeof message !== 'string' || message.length <= 3) throw new TypeError('Must specify message. Message must be a string with length bigger than 3');
    console.info(`[Transaction rollback] id ${this.id}: ${this.message}`);
