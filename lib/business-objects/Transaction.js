...
class Transaction {
  ...
  */
  ...
  commit(message) {
    if (!message) {
      console.info(`[Transaction commit] id ${this._id}`);
    ...
    if (!this._type) { this._type = Transaction.types.BALANCE_CHANGE; }

    this._message = message;

    console.info(`[Transaction commit] id ${this._id}: (${this.value}p) ${this._message}`);
   ...
   */
  set type(type) {
    const allowedTypes = ['BALANCE_CHANGE', 'BALANCE_SET', 'BALANCE_SYNC'];
