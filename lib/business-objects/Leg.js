...
class Leg {
  ...
  activate() {
    ...
    let bookingAction;
    if (this.leg.booking) {
      // booking already exists, try to reserve
      bookingAction = this.leg.booking.reserve();
    } else if (!this._isBookableLeg()) {
      // leg that does not need booking
      bookingAction = Promise.resolve();
    } else {
      // acquire booking, then try to reserve

      // TODO: Search if there is already suitable ticket to use for this leg to reuse

      // create new booking
      const legData = this._toBookingSpecificLeg();
      // as leg is already PAID, set booking fare as zero
      const fare = { amount: 0, currency: 'POINT' };
      // set starting time to be now (per leg activation)
      legData.startTime = Date.now();

      bookingAction = Booking.create({ leg: legData, fare }, this.identityId)
        .then(booking => {
    ...
    return bookingAction
      ...
      .catch(err => {
        ...
        console.warn(`[Leg] '${this.leg.id}' Failed to activate; error with booking!`);
        console.warn(err.stack);
        return this._refund()
          .then(() => this._changeState('CANCELLED_WITH_ERRORS'));
      })
      .then(() => models.Leg
        .query()
        .patch({ state: this.leg.state })
  ...
  */
  ...
  /**
   * Refund leg fare to user
   *
   * @private
   */
  _refund() {
    let fare = this.leg.fare
    ...
    return Profile.retrieve(this.identityId)
      .then(profile => {
        // save current balance for possible refund
        const restoredBalance = profile.balance + fare;
        console.info(`[Leg] Refunding ${fare} points for the user`);
        return Profile.update(this.identityId, { balance: restoredBalance })
