    ...
    return Promise.all([
      ...
      .spread((allBookings, optionsResult) => {
        ...
        for (let i = 0; i < allBookings.length; ++i) {
          ...
          const bookingMeta = booking.meta[this.leg.agencyId.toLowerCase()];
          const validUntil = bookingMeta &&
                             bookingMeta.validFrom &&
                             bookingMeta.validSeconds &&
                             bookingMeta.validFrom + (bookingMeta.validSeconds * 1000);
          if (!validUntil) {
            continue;
          }
          ...
          const optionMatch = optionsResult.options.some(option => {
            const optionMeta = option.meta[this.leg.agencyId.toLowerCase()];
            return optionMeta.name === bookingMeta.type;
          });
          if (optionMatch) {
