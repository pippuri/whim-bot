...
class Leg {
      ...
      .then(leg => {
        ...
        if (leg._isBookableLeg()) {
          return Booking.isAgencyIdSupported(leg.leg.agencyId)
            .then(isSupported => {
              if (isSupported !== true) {
                return Promise.reject(new MaaSError(`AgencyId '${leg.leg.agencyId}' not supported`, 400));
              }
  ...
  reserve() {
    ...
    if (!this.leg.booking) {
      ...
      if (this.leg.booking.state === 'RESERVED') {
        return Promise.reject(new MaaSError('Already \'RESERVED\' booking for this leg', 400));
      }
