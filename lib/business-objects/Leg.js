...
class Leg {
  ...
  activate() {
    ...
    if (!this._isValidStateChange('ACTIVATED')) {
      return Promise.reject(new MaaSError(`This leg cannot be activated because now '${this.leg.state}'`, 400));
    }
    ...
    return bookingReserve
      ...
      .catch(err => {
        ...
        console.error(`[Leg] '${this.leg.id}' Failed to active; error with booking!`);
      ...
      .catch(err => {
        console.error(`[Leg] Failed activating of '${this.leg.id}':`, err);
        return Promise.reject(new Error(`Failed to activate leg '${this.leg.id}':` + err));
      });
  }
  ...
  finish() {
    ...
    if (!this._isValidStateChange('FINISHED')) {
      return Promise.reject(new MaaSError(`This leg cannot be finished because now '${this.leg.state}'`, 400));
    }
