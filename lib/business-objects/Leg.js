...
class Leg {
  ...
  activate() {
    ...
    let bookingAction;
    if (this.leg.booking) {
      // booking already exists, try to pay & reserve
      bookingAction = this.leg.booking.pay({ skipPayment: true })
      ...
      bookingAction = Booking.create(bookingInput, this.identityId)
        .then(booking => {
        ...
        .then(() => models.Leg
          ...
          .where('id', this.leg.id))
        .then(() => this.leg.booking.pay({ skipPayment: true }))
        .then(() => this.leg.booking.reserve());
    }
