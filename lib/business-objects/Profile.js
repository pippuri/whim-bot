...
class Profile {
  ...
  static create(identityId, phone, transaction) {
    ...
    if (!transaction) return Promise.reject(new Error('New transaction is required to create new user profile'));
    ...
    .then(responses => {
      ...
      if (exists) {
        return Promise.reject(new MaaSError('[Profile] Profile already exists', 400));
      }

      // We got an existing Chargebee profile -> return that
      if (chargebeeUser !== null) {
        console.info('[Profile] Creating a new profile with an existing Chargebee user');
    ...
    .then(chargebeeUser => {
      return subscriptionMgr.getPlanById(chargebeeUser.plan.id)
        .catch(error => {
          console.warn(`[Profile] Error in fetching Chargebee plan: '${JSON.stringify(error, null, 2)}'`);
          const message = `[Profile] Error in fetching Chargebee plan '${chargebeeUser.plan.id}'`;
