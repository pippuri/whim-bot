...
function deleteUserCognitoProfile(identityId) {
  ...
  return cognitoSync.deleteDataset({
...
class Profile {
  ...
  static addFavoriteLocation(identityId, location, transaction) {
    console.info(`[Profile] Adding favorite location '${location.name}' for '${identityId}'`);

    return Profile.retrieve(identityId, ['favoriteLocations'])
      .then(profile => {
        if (profile.favoriteLocations.some(item => item.name === location.name)) {
          const message = `Favourite location '${location.name}' already exists`;
          return Promise.reject(new MaaSError(message, 409));
        }
