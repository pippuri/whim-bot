...
class Profile {
   ...
   *
   * @param {string} identityId - The User identity to updateSubscription
   ...
   * @param {boolean} [skipUpdate=false] - Whether or not skip Chargebee update
   * @return {Promise} resolving to updated Profile, or reject with Error
   */
  static updateSubscription(identityId, planId, transaction, promoCode, skipUpdate, forceUpdate) {
    console.info(`[Profile] Set new subscription plan '${planId}' for identity '${identityId}'`);
    ...
    return Profile.retrieve(identityId)
      ...
      .then(update => {
        return transaction.meta('planId', planId)
          .then(() => (promoCode ? transaction.meta('promoCode', promoCode) : Promise.resolve()))
