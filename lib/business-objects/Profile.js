...
class Profile {
  ...
  /**
   * Renews the subscription plan.
   * When renewing plans, the points are reset to whatever the given plan is.
   */
  static renewSubscription(identityId, planId) {
    console.info(`[Profile] Set new subscription plan '${planId}' for identity '${identityId}'`);
   ...
   */
  static changeSubscription(identityId, planId, transaction) {
    console.info(`[Profile] Set new subscription plan '${planId}' for identity '${identityId}'`);
    ...
    return Profile.retrieve(identityId)
      ...
      .then(plan => {
        ...
        return transaction.meta('planId', planId)
          .then(() => Profile.update(identityId, { balance: newBalance }, transaction));
      })
      .then(response => {
   ...
   * @return {Promise} resolving to updated Profile, or reject with Error
   */
  static issueSubscriptionChange(identityId, planId, promoCode) {
