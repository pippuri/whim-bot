...
class Booking {
   ...
   */
  static create(bookingInput, identityId, options) {
    ...
    if (skipInsert === true && typeof trx !== 'undefined') {
      return Promise.reject(new Error('both skipInsert and transaction cannot be used'));
    }
   ...
   */
  static webhookCallback(agencyId, tspId, payload) {
    ...
    return models.Booking.query()
      ...
      .then(response => response[0]); // Assume that there is only 1 tspId therefore there is one row that is affected
  }
   ...
   * Pay booking
   *
   * @param {boolean} [options.skipPayment] - Set true not to actually deduct the set fare (e.g. if already
   * paid in itinerary level)
   * @param {Object} [options.trx] Objection transaction object. If given, all database operations here
   * are bind to this transaction. Whoever calls this pay-method, must then commit or rollback the transaction!
   *
   * @return {Promise -> Booking}
  ...
  cancel(options) {
    return this.assignTransport()
      .then(() => this._cancelWithTSP(options));
  }

  _cancelWithTSP(options) {
    options = options !== undefined ? options : {};
