...
class Booking {
  ...
  validateOwnership(identityId) {
    return Promise.resolve() //validator.validate('core:aws-units#definitions/identityId', identityId)
      .then(_result => {
  ...
  _refreshWithTSP() {
    ...
    if (this.booking.state !== 'RESERVED') {
      return Promise.reject(new MaaSError(`Booking '${this.booking.id}' cannot be refreshed because now '${this.booking.state}'`, 400));
    }
  ...
  pay(trx) {
    ...
    const originalState = this.booking.state;
    ...
    return maasOperation.fetchCustomerProfile(this.booking.customer.identityId) // Get customer information
      ...
      .catch(err => {
        console.error('[Booking] failed to pay:' + err);
        this.booking.state = originalState;
        let rollback;
        if (oldBalance) {
          console.log(`[Booking] Restoring original ${oldBalance} points for customer...`);
          rollback = maasOperation.updateBalance(this.booking.customer.identityId, oldBalance);
        } else {
  ...
  _cancelWithTSP() {
    let cancelTargetState;
    if (this.booking.state === 'PAID' && this._isValidStateChange('REJECTED')) {
      cancelTargetState = 'REJECTED';
    } else if (this._isValidStateChange('CANCELLED') && this._isValidStateChange('CANCELLED_WITH_ERRORS')) {
      cancelTargetState = 'CANCELLED';
    }

    if (!cancelTargetState) {
    ...
    let tspCancelAction;
    if (cancelTargetState === 'CANCELLED') {
      tspCancelAction = this.tsp.cancel(this.booking.id);
    } else {
    ...
    return tspCancelAction
      .then(() => {
        return this._changeState(cancelTargetState);
      })
      .catch(error => {
        errorMessage = `Booking ${this.booking.id}, agencyId ${this.booking.leg.agencyId}, cancellation failed: ${error.message}`;
        ...
        cancelTargetState = 'CANCELLED_WITH_ERRORS';
        return this._changeState(cancelTargetState);
      })
      .then(() => {
        return models.Booking.query()
      ...
      .then(updateCount => {
        ...
        console.info(`[Booking] Cancellation success of '${this.booking.id}', agencyId ${this.booking.leg.agencyId}.`);
        return Promise.resolve(this);
      });
  }
