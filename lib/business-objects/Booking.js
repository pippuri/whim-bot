    ...
    return maasOperation.fetchCustomerProfile(this.booking.customer.identityId) // Get customer information
      .then(profile => {
        ...
        return this.tsp.reserve(reservation)
          ...
          .catch(error => {
            ...
            errorMessage = `Could not make a reservation: ${error.message}`;
            return Promise.all([
      ...
      .then(updateCount => {
        ...
        if (this.booking.state === 'REJECTED') {
          return Promise.reject(new MaaSError(`Booking ${this.booking.id} reservation failed: ` + errorMessage, 400));
        }
