...
class Booking {
  ...
  static create(bookingInput, identityId, transaction, options) {
    ...
    return Promise.resolve()
      ...
      .then(booking => {
        ...
        transaction.meta(models.Booking.tableName, booking.booking.id);
        return transaction.bind(models.Booking)
          .query()
   ...
   * @param {object} [transaction] - The transaction context to use
   *
   * @return {Promise -> Booking}
   */
  static retrieve(bookingId, transaction) {

    const model = (transaction) ? transaction.bind(models.Booking) : models.Booking;

    return validator.validate(unitsSchema.definitions.uuid, bookingId)
      .then(_result => model.query()
      .findById(bookingId))
      .then(bookingData => {
        if (!bookingData) {
  ...
  static query(identityId, startTime, endTime, states) {
    ...
    return query
      ...
      .then(results => Promise.all(results.map(bookingData => {
        return Promise.resolve(new Booking(bookingData))
          .then(booking => booking.assignTransport());
      })));
  }
   ...
   *
   * @param {object} transaction - The transaction context for refreshing
   * @param {boolean} [options.skipRefund] - Set true not to refund in case of booking gets in
   *   failure state when refreshing
   * @return {Promise -> Booking}
   */
  refresh(transaction, options) {
    return this.assignTransport()
      .then(() => this._refreshWithTSP(transaction));
  }

  _refreshWithTSP(transaction, options) {
    ...
    if (typeof skipRefund !== 'boolean') {
      return Promise.reject(new Error('skipRefund not true/false', 400));
    }
    ...
    const fare = (this.booking.fare && this.booking.fare.amount) || 0;

    return this.tsp.retrieve(this.booking.tspId)
      .catch(error => {
        // Handle error 503 for temporary error - retain the booking in the current state
        // TODO Add error enums for TSPErrors
        if (error.message === 'Temporary error communicating with TSP') {
          return { state: this.booking.state };
        }
      ...
      .then(verifiedBooking => {
        ...
        // Update state as TSP has changed it
        console.info(`[Booking] TSP changed state '${this.booking.state}' -> '${verifiedBooking.state}'`);
        // check if eligible for refunding (TSP has cancelled the booking)
        if (skipRefund === false && fare !== 0 &&
            ['PAID', 'RESERVED', 'CONFIRMED', 'ACTIVATED'].some(state => state === this.booking.state) &&
            ['CANCELLED', 'CANCELLED_WITH_ERRORS', 'REJECTED'].some(state => state === verifiedBooking.state)) {
          console.info(`[Booking] Refunding fare of ${fare} points due to booking issues`);
          let ticketDesc = this.getTicketDescription();
          ticketDesc = !ticketDesc ? '' : `${ticketDesc} `;
          transaction.message = `Refunding booking ${ticketDesc}fare due to issues with the booking`;
          return Profile.increaseBalance(this.booking.customer.identityId, fare, transaction)
            .then(() => this._changeState(verifiedBooking.state))
            .then(() => verifiedBooking);
        }
      ...
      .then(() => {
        // TODO Only change the DB data if the booking has changed
        // persist and return new booking
        transaction.meta(models.Booking.tableName, this.booking.id);
        return transaction.bind(models.Booking)
          .query()
  ...
  pay(transaction, options) {
    ...
    return Profile.retrieve(this.booking.customer.identityId)
      .then(profile => {
        ...
        console.info(`[Booking] Paying ${this.booking.fare.amount} points`);
        return Profile.decreaseBalance(this.booking.customer.identityId, this.booking.fare.amount, transaction);
      })
   ...
   *
   * @return {Promise -> Booking}
   * @param {boolean} [options.skipRefund] - Set true not to refund in case of reservation failure
   */
  reserve(transaction, options) {
    return this.assignTransport()
      .then(() => this._reserveWithTSP(transaction, options));
  }

  _reserveWithTSP(transaction, options) {
    if (!transaction) {
      return Promise.reject(new Error('a started transaction is required for booking creation'));
    }
    ...
    return Profile.retrieve(this.booking.customer.identityId)
      .then(profile => {
        // create reservation object for TSP, but don't send identityId, agencyId
        const customer = utils.cloneDeep(this.booking.customer);
        delete customer.identityId;
        const leg = utils.cloneDeep(this.booking.leg);
        delete leg.agencyId;

        // Set terms from leg product data. However, if there are terms already
        // defined in booking object, those will override.
        // TODO: Move e.g. Whim car booking.terms.type under bookig.product
        const terms = Object.assign({}, this.booking.product, this.booking.terms);
  ...
  /**
   * Generates Booking ticket description string. Returns e.g. "HSL Seutu" or
   * "Valopilkku".
   *
   * @return {string}
   * @return {null} in case we cannot return any description
   */
  getTicketDescription() {
    let desc = this.booking.terms.agencyId || this.booking.leg.agencyId || '';
    desc += desc && this.booking.terms.type ? ' ' + this.booking.terms.type : '';
    ...
    return Profile.retrieve(this.booking.customer.identityId)
      .then(profile => {
        this.booking.customer = Object.assign(this.booking.customer, {
          firstName: profile.firstName || 'Whim',
   ...
   */
  get fare() {
    return Object.assign({}, this.booking.fare);
  }
