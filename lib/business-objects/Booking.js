  ...
  pay(trx) {
    ...
    return maasOperation.fetchCustomerProfile(this.booking.customer.identityId) // Get customer information
      .then(profile => {
        ...
        return maasOperation.computeBalance(this.booking.fare.amount, profile)
          // Deduction
          ...
          .catch(error => {
            errorMessage = `Updating balance failed: ${error.message}`;
            console.warn(errorMessage);
            console.warn(`Booking: ${JSON.stringify(this.booking, null, 2)}`);
      ...
      .then(updateCount => {
        ...
        // In case we didn't move to PAID state, there must have been an error in
        // process - caught or not. For caught ones, errorMessage is initialised.
        return Promise.reject(new Error(errorMessage));
      })
      .catch(err => {
        console.error(`[Booking] FAILED payment of '${this.booking.id}': ${err.message}`);
        ...
        return this.tsp.reserve(reservation)
          ...
          .catch(error => {
            ...
            return this._changeState('REJECTED')
              .then(() => {
                if (fare !== 0) {
                  const oldBalance = currentBalance + fare;
                  console.info(`[Booking] Refunding ${fare} points for the user`);
                  return maasOperation.updateBalance(this.booking.customer.identityId, oldBalance); // Refunding
                }
                return Promise.resolve();
              });
          });
    ...
    return cancelAction
      ...
      .then(() => {
        ...
        if (fare !== 0 && this.state === 'CANCELLED') {
          ...
          return maasOperation.fetchCustomerProfile(this.booking.customer.identityId)
            .then(profile => maasOperation.updateBalance(this.booking.customer.identityId, profile.balance + fare));
        }
        return Promise.resolve();
      })
      .then(() => {
